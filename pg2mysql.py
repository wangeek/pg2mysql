...
def exportCSV(con, table_str):
    ...
    try:
        cur = con.cursor()
	io = open("pg2mysql_tabledata_%s.csv" % table_str, "w")
...
def main(pg_db_name, pg_user, pg_passwd, pg_server="localhost"):
	...
	for table in cursor.fetchall():
		...
		load_data += "LOAD DATA LOCAL INFILE 'pg2mysql_tabledata_%s.csv' INTO TABLE %s;\n" % (table[0], table[0])
		...
		for column in get_table_col_names(conn, table[0]):
			...
			logging.debug("%s.%s: %s - %s - %s - %s" % (table[0], column_name, character_maximum_length, column_default, data_type, is_nullable))
...
if __name__ == "__main__":
	...
	parser.add_option("-H", "--host",  
	                  action="store", type="string", dest="pg_server") 
	parser.add_option("-D", "--db",  
	                  action="store", type="string", dest="pg_db_name") 
	parser.add_option("-U", "--user",  
	                  action="store", type="string", dest="pg_user") 
	parser.add_option("-P", "--password",  
	                  action="store", type="string", dest="pg_passwd") 
	
	(options, args) = parser.parse_args() 

	main(options.pg_db_name, options.pg_user, options.pg_passwd)
